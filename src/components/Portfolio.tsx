...
export default function Portfolio() {
  ...
  return <section id="portfolio" className="relative py-12 lg:py-16 border-y border-white/50 bg-white/20 backdrop-blur-sm">
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        {portfolioData.map((item,index)=>{
          ...
          return <motion.div key={item.id} initial={{opacity:0,y:40}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-100px'}} transition={{duration:.8,delay:index*.1,ease:[.16,1,.3,1]}} className={`group ${item.link?'cursor-pointer':''} ${index%2!==0?'md:mt-10 lg:mt-16':''}`}>
